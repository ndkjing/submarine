...
class ComData:
    ...
    def read_sub_data(self, debug=False):
        """
        读取水下机器人数据返回
        @param debug:True 答应获取到的数据
        @return: [经度, 纬度, 误差（米）, 速度, 航向, 磁偏角]
        ...
        time.sleep(0.05)
        try:
            sub_data = self.readline()
            str_data = bytes(sub_data).decode('ascii')
            find_str = re.findall('[PSD].*?Z', str_data)
            if len(find_str)>0 and len(find_str[0])>0:
                str_data = find_str[0]
                str_data = str_data.strip()
                print('read_sub_data:',str_data,len(str_data))
                # print('rtk readline str_data', str_data)
                if str_data.startswith('P'):
                    ...
                    try:
                        ...
                        p = int(data_list1[1])
                        y = int(data_list1[2])
                        lng = float(data_list1[3])
                        lat = float(data_list1[4])
                        return {'pos': [r, p, y, lng, lat]}
                    except Exception as sub_pos_error:
                        if debug:
                            print({'error sub_pos_error': sub_pos_error})
                if str_data.startswith('S'):

                    data_list1 = str_data.split(',')
                    try:
                        switch_list = [int(i) for i in data_list1]
                        return {'switch': switch_list}
                    except Exception as switch_list_error:
                        if debug:
                            print({'error switch_listr': switch_list_error})
                if str_data.startswith('D'):
                    str_data = str_data[1:-1]
                    data_list = str_data.split(',')
                    try:
                        deep = float(data_list[0])
                        tem = round(float(data_list[1]))
                        return {'deep': deep, 'tem': tem}
                    except Exception as deep_error:
                        if debug:
                            print({'error deep_error': deep_error})
                if str_data.startswith('E'):
                    str_data = str_data[1:-2]
                    try:
                        energy = round(float(str_data))
    ...
    while True:
        try:
            ...
            if int(check_type) == 1:
                ...
                b_laser = 1
            if b_com_data:
                print('config.stc_port config.stc_baud', config.stc_port, config.stc_baud)
